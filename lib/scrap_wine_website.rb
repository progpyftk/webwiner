...
class WineWebsite
  ...
  START_PAGE_NUMBER = 1 # the to start scraping, in order to make test, increase it
  ...
  def get_product_data(link)
    ...
    while index < doc.xpath(WINE_DATA_FEATURE_NAME).length
      ...
      if doc.xpath(WINE_DATA_FEATURE_NAME)[index].text == 'Uva'
        wine_data[:grape] =
          doc.xpath(WINE_DATA_FEATURE_VALUE)[index].text
      end
      ...
      if doc.xpath(WINE_DATA_FEATURE_NAME)[index].text == 'Safra'
        wine_data[:year] =
          doc.xpath(WINE_DATA_FEATURE_VALUE)[index].text.to_i
      end
    ...
    # getting the name
    wine_data[:name] = doc.xpath(WINE_NAME_XPATH).first.text unless doc.xpath(WINE_NAME_XPATH).first.nil?
    puts "year antes do tratamento: #{wine_data[:year]}"
    puts "nome: #{wine_data[:name]}"

    if !wine_data[:name].nil? && wine_data[:year].nil?
      year = wine_data[:name].scan(/\d{4}/) # it might return an array
      wine_data[:year] = year.to_i if year.length == 4
    end
